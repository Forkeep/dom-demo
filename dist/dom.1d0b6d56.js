...
window.dom = {
  cerate: function cerate(crateSting) {
    var contanier = document.createElement('template');
    contanier.innerHTML = crateSting.trim();
    return contanier.content.firstChild;
  },
  after: function after(node, addNode) {
    node.parentNode.insertBefore(addNode, node.nextSibling);
  },
  before: function before(node, addNode) {
  ...
  style: function style(node, nameOrObject, value) {
    if (arguments.length === 1) {
      // return node.style.cssText.split(';')
      return node.style.cssText.trim().split(';').filter(function (e) {
        return e !== "";
      });
    } else if (arguments.length === 2) {
  ...
  on: function on(node, eventName, fn, captureOrBubble) {
  ...
  off: function off(node, eventName, fn, captureOrBubble) {
    if (arguments.length === 3) {
